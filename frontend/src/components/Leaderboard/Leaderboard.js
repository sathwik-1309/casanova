...
function LeaderboardItem({data, index}) {
  return (
    <div className='flex-row default-font font-600 font-1 h-50 bg-shadow mt-1 rounded-2'>
      ...
      <div className={`${data.color}1 pl-10 w-200 flex vert-align`}>{data.name}</div>
      <div className={`${data.color}1 opacity-10 flex-centered w-60`}>{data.matches}</div>
...
function HistoryItem({data, index}) {
  return (
    <div className='flex-row default-font font-600 font-1 h-50 bg-shadow mt-1 rounded-2'>
      ...
      <div className={`${data.color}1 opacity-10 flex-centered w-60`}>{data.matches}</div>
      <div className={`${data.color}1 opacity-9 flex-centered w-100`}>{data.highest_rating}</div>
      <div className={`${data.color}1 opacity-10 flex-centered w-60 font-0_9 font-500`}>{data.match_id}</div>
    </div>
  )
}
...
export function Leaderboard () {
  let url = `${BACKEND_API_URL}/player/leaderboard`
    const [data, setData] = useState(null);
    const [format, setFormat] = useState('wt20')
    const [display, setDisplay] = useState('bat')
  ...
  return (
    <div className='flex-col default-font flex-centered'>
      ...
      <div className='flex-row bg-white bg-shadow pb-15 pt-15 w-600 wrap gap-50 flex-centered mt-30 font-0_9'>
        <div className={`${display == 'bat' ? 'display_select' : 'display_unselect'} w-150 h-30 font-600 flex-centered rounded-4`} onClick={()=>{setDisplay('bat')}}>BATTING</div>
        <div className={`${display == 'ball' ? 'display_select' : 'display_unselect'} w-150 h-30 font-600 flex-centered rounded-4`} onClick={()=>{setDisplay('ball')}}>BOWLING</div>
        <div className={`${display == 'all' ? 'display_select' : 'display_unselect'} w-150 h-30 font-600 flex-centered rounded-4`} onClick={()=>{setDisplay('all')}}>ALL-ROUNDERS</div>
      </div>

      <div className='flex-row'>
        <History data={data[format][display]} />
        <LeaderboardList data={data[format][display]} />
      </div>
